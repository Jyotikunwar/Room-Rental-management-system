...
export default function FeaturedRooms({ onViewDetails, onBrowseRooms }: FeaturedRoomsProps) {
  ...
  async function loadRooms() {
    ...
    try {
      ...
      console.error("Failed to load featured rooms from backend:", e);
      setAllRooms([]);
    } finally {
      setLoading(false);
    }
  ...
  const displayedRooms = showAll ? allRooms : allRooms.slice(0, 8);

  const handleViewAllClick = () => {
    setShowAll((prev) => !prev);
    if (onBrowseRooms) onBrowseRooms();
  };
    ...
    <section id="featured-rooms" className="mx-auto max-w-7xl px-6 py-16">
      <div className="mb-8 flex items-end justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 sm:text-3xl">
            {showAll ? `All Available Rooms (${allRooms.length})` : "Featured Rooms & Apartments"}
          </h2>
          <p className="mt-1 text-sm text-gray-500">
            {showAll
              ? "Browse every active room and apartment listing from the database"
              : "Explore top available rooms across Kathmandu, Lalitpur, and Pokhara"}
          </p>
        </div>
        <button
          onClick={handleViewAllClick}
          className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-semibold text-blue-700 transition hover:bg-blue-600 hover:text-white flex items-center gap-1 cursor-pointer shadow-xs"
        >
          {showAll ? "Show Featured Only ↑" : `View all rooms (${allRooms.length}) →`}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {loading ? (
          ...
          <p className="col-span-full py-10 text-center text-gray-400">No rooms listed in database yet.</p>
        ) : (
